...
//
// NOTE (redesign port): richSanitize + the whitelist below become the site-side
// js/site/richtext-sanitize.js. The block's stored value is `text.html`; the
// site renderer must re-sanitize on render (never trust stored HTML blindly).
...
// NOTE (redesign port): the site renderer (js/site/blocks.js) can inject the
// sanitized HTML directly into a container instead of pretty-printing to a string.
