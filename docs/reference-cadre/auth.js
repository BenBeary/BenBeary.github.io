...
// NOTE (redesign port): change these to the user's own account/repo. Owner-only,
// so the collaborator/permission checks below can be simplified to an owner check.
...
// NOTE (redesign port): the basic/admin page-role gate is CADRE-specific -
// delete it and this redirect logic when porting.
...
async function validateAndStorePAT(pat, persistent) {
    ...
    // NOTE (redesign port): for an owner-owned repo you can replace this whole
    // block with an owner check: `if (user.login !== GITHUB_OWNER) throw ...`.
...
function renderAuthUI() {
    ...
    if (user) {
        chip.innerHTML = '<div class="auth-user">'
            ...
            + renderExpiryWarning()
            + '<button class="btn-header-action" id="btn-sign-out" title="Sign out">Sign out</button>'
            + '</div>';
    } else {
...
// NOTE (redesign port): the whole page-role gate is CADRE-specific - remove it.
